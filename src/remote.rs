                ...
                if let Ok((len, from)) = r {
                    if let Ok((_, packet)) = rosc::decoder::decode_udp(&osc_buf[..len]) {
                        if let Some(ad) = osc_msg::parse_advertisement(&packet) {
                            handle_advertisement(&mut roster, ad, from.ip(), group_name, node_id, &member_set);
                        }
                    }
                }
            }
...
fn handle_advertisement(
    ...
    roster.insert(
        ...
        Member {
            ...
            peer_inputs: n_in,
            in_seq: 0,
            return_jitter: JitterBuffer::new(),
        ...
        tokio::select! {
            ...
            r = osc_socket.recv_from(&mut osc_buf) => {
                if let Ok((len, from)) = r {
                    if let Ok((_, packet)) = rosc::decoder::decode_udp(&osc_buf[..len]) {
                        if let Some(ad) = osc_msg::parse_advertisement(&packet) {
                            handle_advertisement_macos(&mut roster, &mut chans, ad, from.ip(), group_name, node_id, &member_set);
                        }
                    }
                }
            }
...
fn handle_advertisement_macos(
    ...
    if let Some(existing) = roster.get_mut(&src_ip) {
        ...
        existing.out_jitter = JitterBuffer::new();
        existing.return_jitter = JitterBuffer::new();
        existing.in_seq = 0;
        existing.last_seen = Instant::now();
        return;
    }
